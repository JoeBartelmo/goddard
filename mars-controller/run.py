...
def initOutput(config, q = None):
    ...
    if not os.path.exists(config.logging.output_path + '/output/' + timestamp + '/'):
        os.makedirs(config.logging.output_path + '/output/' + timestamp + '/')

    logger = logging.getLogger()
    logger.setLevel(logging.INFO)

    # create debug file handler and set level to debug
    handler = logging.FileHandler(os.path.join(config.logging.output_path + '/output/' + timestamp + '/', "log.txt"),"w")
    handler.setLevel(logging.INFO)
    formatter = logging.Formatter("%(levelname)s - %(message)s")
    handler.setFormatter(formatter)
    logger.addHandler(handler)

    if (os.path.isfile(config.logging.output_path + '/output/' + timestamp + '/' + config.logging.log_name + '_machine_log.csv')):
